...
<div class="panel-body">
    ...
    <script type="text/javascript">
        ...
        function loadCommunity(communityId) {
            $.get("${g.createLink(controller:'map', action:'loadCommunityUserMap')}/?community.id=" + communityId, function (data) {
                //html = "";
        ...
        function loadCommunityMap( name,id) {
            ...
            if (check) {
                ...
                var url;
                if (id) {
                    url="${g.createLink(controller:'map', action:'communityXml')}/?id="+id
                } else {
                    url="${g.createLink(controller:'map', action:'communityXml')}/?name="+name
                }
        ...
        var imageNormal = new google.maps.MarkerImage(
                "${asset.assetPath(src:'map/square.png')}",
                new google.maps.Size(11, 11),
                new google.maps.Point(0, 0),
                new google.maps.Point(6, 6)
        );
        var imageHover = new google.maps.MarkerImage(
                "${asset.assetPath(src:'map/square_over.png')}",
                new google.maps.Size(11, 11),
                new google.maps.Point(0, 0),
                new google.maps.Point(6, 6)
        );
        var imageNormalMidpoint = new google.maps.MarkerImage(
                "${asset.assetPath(src:'map/square_transparent.png')}",
                new google.maps.Size(11, 11),
                new google.maps.Point(0, 0),
                new google.maps.Point(6, 6)
        );
        ...
        <g:set var="firstEntry" value="${currentEntry}"/>
        overlayname.push("${firstEntry.name}");
        var myLatlng = ${firstEntry.firstgoogleMapString};
        var myLatlng1 = new google.maps.LatLng(-7,-38);
        thisshape.push("Polygon");
        description.push('');
        jsfromphp = [[${firstEntry.googleMapString}]]
        ...
        function loadmap() {
            ...
            //var myLatlng = new google.maps.LatLng(-7,-38); // ${firstEntry.firstgoogleMapString}; // value comes from php script
            var myLatlng = ${firstEntry.firstgoogleMapString}; // value comes from php script
            var myOptions = {
                zoom: 10,
                center: myLatlng,
             ...
             //map = new google.maps.Map(gob('map_canvas'),myOptions);
                map = new google.maps.Map(document.getElementById("communityMap"), myOptions);
            ...
            */
            ...
            addOverlayFromKML('${firstEntry.name}');
        ...
        function showAddress(address) {
            geocoder.geocode({'address': address}, function(results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
                    ...
                    loadCommunityByGeo(pos.lat(),pos.lng())
        ...
        function loadCommunityByGeo(lat,lng) {
            $.ajax({
                type: 'POST',
        ...
        //]]>
        function addOverlayFromKML(name) {
            //var linepoints = [];
            ...
            $.ajax({
                ...
                error: function() {
                    tinyMarker = new google.maps.Marker({
                        position: ${firstEntry.firstgoogleMapString},
                        map: map,
                        icon: tinyIcon,
                        label: { color: '#000', fontWeight: 'bold', fontSize: '14px', text: name, display:'block', background:'#FFF' }
                    });
        ...
        function addOverlayFromKML1(name) {
        ...
        function markerstyleobject() {
            ...
            this.icon = "${asset.assetPath(src:'/map/red-dot.png')}";
        }
